...
GRN=$(tput setaf 2)     # for announcing explicit commands we'll run
YLW=$(tput setaf 3)     # for output of non-git commands and also
                            # showing the explicit command requested
                            # prompts (in bold)
                            # info about errors causing exit
                            # user todo at end of script
TEAL=$(tput setaf 6)    # for titles announcing what we're doing
...
echo "${TEAL}## Testing uniqueness of source files${RESET}"
...
echo "${TEAL}## Testing all targets are commands or non-existent files${RESET}"
...
[ -e $ctrlfile.md ] && echo "${BOLD}${YLW}note: $ctrlfile.md already exists. I will overwrite in 3 seconds... (^c to quit)${RESET}" && sleep 5
echo "
...
while read srcfile tgtfile msg ; do
    case $srcfile in
        "$")
            ...
            ;;
        *)
            echo "${GRN}:: cp+git add: ${YLW}$(cp -av $srcfile $tgtfile)${RESET}"
            git add $tgtfile
            if [ -n "$msg" ] ; then
...
echo "${TEAL}## Your todo: manual review/cleanup:
* review git log. If satisfied then remove original source files${RESET}"
echo "${BOLD}${YLW}tar cvfz chronogitcsv2git-timemachine.tgz $ctrlfile.csv $ctrlfile.md $srcflist
rm $srcflist
git rm $ctrlfile.csv
git rm $ctrlfile.md${RESET}"
echo "${TEAL}* Update README.md to suit
